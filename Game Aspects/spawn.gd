...
@export_range(0, 1) var spawn_velocity: float = 5

@export_category("Spawn Properties")
@export var spawn_appearance : PackedScene
@export var spawn_is_everlasting = false
@export var spawn_lifetime : float = 10
@export var spawn_is_damaging = false
@export var spawn_is_collectible = true

@export_category("Spawn Type Settings")
@export_group("Damage Settings")
@export var damage_instakills = true
@export_group("Collectible Settings")
@export var collectible_points = 1
@export var collectible_adds_time = false
@export var collectible_opens_door = false
...
func _ready():
	...
	if !spawn_is_everlasting:
		await get_tree().create_timer(spawn_lifetime).timeout
		queue_free()
